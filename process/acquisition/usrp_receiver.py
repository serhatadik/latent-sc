"""
USRP Radio Receiver for RF Data Acquisition.
Migrated from usrp_receive_samples.py with minimal changes.

This script interfaces with USRP hardware to receive IQ samples and save them
to .npy files with timestamps.
"""
import time
import uhd
import datetime
...
class Radio():
    """Interface for USRP radio hardware."""

    RX_CLEAR_COUNT = 1000
    LO_ADJ = 1e6

    def __init__(self) -> None:
        """Initialize USRP radio and configure streaming."""
    ...
    def tune(self, freq, gain, rate, use_lo_offset=False, gpiomode=None):
        """
        Configure radio frequency, gain, and sample rate.

        Args:
            freq: Center frequency in Hz
            gain: Gain in dB
            rate: Sample rate in Hz
            use_lo_offset: Use LO offset to avoid DC spike
            gpiomode: GPIO mode configuration (optional)
        """
    ...
    def recv_samples(self, nsamps, start_time=None, rate=None):
        ...
        Args:
            ...
            start_time: Time to start receiving (for synchronization)
            rate: Sample rate (if different from current)

        Returns:
            Tuple of (samples, timestamp)
        """
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Receive IQ samples from USRP and save to files"
    )
    parser.add_argument(
        "-f", "--frequency",
        help="Frequency to receive samples",
        default=3534e6,
        type=float
    )
    parser.add_argument(
